...
mod tests {
    ...
    fn test_get_jwt_secret() {
        ...
        assert!(secret.is_ok(), "Failed to get JWT secret: {:?}", secret.err());
        assert_eq!(secret.unwrap(), "b\"secret\"".to_string().into_bytes(),
    ...
    fn test_get_jwt_timeout() {
        ...
        assert!(timeout.is_ok(), "Failed to get JWT timeout: {:?}", timeout.err());
        assert_eq!(timeout.unwrap(), 3600, "JWT timeout does not match expected value");
    }
    ...
    fn test_get_user_id_from_jwt() {
        ...
        assert_eq!(user_id.unwrap(), USER_ID, "User ID does not match expected value");
    }
